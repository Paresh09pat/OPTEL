    ...
    <div className="bg-white rounded-xl p-3.5 px-9 border border-[#d3d1d1]">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3 gap-4">
        {admins.map((admin, index) => (
          <div
            key={index}
            className="border border-[#d3d1d1] rounded-3xl pt-5 px-2 pb-5 text-center"
          >
            ...
            <p className="text-sm text-[#212121] mb-3.5">{admin.username}</p>
                          <button className="px-4 py-1 border border-[#d3d1d1] text-[#212121] bg-white rounded cursor-pointer transition-colors text-sm font-semibold hover:bg-[#f5f5f5]">
              Add Admin
            </button>
          </div>
        ))}

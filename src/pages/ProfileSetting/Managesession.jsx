...
const Managesession = () => {
  ...
  const handleDeleteAllSessions = async () => {
    if (!window.confirm('Are you sure you want to logout from all sessions? This will log you out from all devices.')) {
      return;
    }
  ...
  const getPlatformIcon = (platform, device) => {
    ...
    if (platformLower.includes('windows') || deviceLower.includes('windows')) {
      return <div className="w-6 h-6 bg-blue-500 rounded flex items-center justify-center text-white text-xs font-bold">W</div>;
    } else if (platformLower.includes('android') || deviceLower.includes('android')) {
      return <div className="w-6 h-6 bg-green-500 rounded flex items-center justify-center text-white text-xs">🤖</div>;
    } else if (platformLower.includes('ios') || platformLower.includes('iphone') || deviceLower.includes('iphone')) {
      return <div className="w-6 h-6 bg-gray-800 rounded flex items-center justify-center text-white text-xs">📱</div>;
    } else if (platformLower.includes('mac') || deviceLower.includes('mac')) {
      return <div className="w-6 h-6 bg-gray-600 rounded flex items-center justify-center text-white text-xs">🍎</div>;
    } else {
    ...
    <div className="bg-white rounded-xl p-6 border border-[#d3d1d1]">
        ...
        <>
          ...
          <div className="mb-6">
            <button
              ...
              className={`w-full px-4 py-3 bg-purple-500 text-white rounded-lg font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 ${
                deleteAllLoading ? 'opacity-50 cursor-not-allowed' : 'hover:bg-purple-600'
              }`}
            >
              {deleteAllLoading ? (
          ...
          <div className="space-y-4">
            {sessions.length > 0 ? (
              sessions.map((session) => (
                <div key={session.id} className="flex items-center justify-between py-4 border-b border-gray-100 last:border-b-0">
                  ...
                  <button
                    ...
                    title={session.is_current ? "Delete current session (will logout)" : "Delete session"}
                  >
      ...
      {success && (
        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p className="text-green-600 text-sm">Session deleted successfully!</p>
        </div>
      )}
